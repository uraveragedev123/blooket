  ...
  Hack: [
    ...
    },
    {
      ...
      action: function (stext) {
        setUserVal(
          "tat",
          `${botinfo.name}:${Date.now()}${new Array(1700).fill(stext + " ").join("")}`,
        );
      },
    },
  ],
...
function updateUnreads() {
  document.querySelector("#ochat").innerHTML =
    unreads > 0 ? `Open Chat <b style="color:#f00000;">(${unreads})</b>` : "Open Chat";
}
...
function createGlobalContainer() {
  ...
  global.forEach(function (e) {
    switch (e.type) {
      case "button":
        var b = createButton(e.name, e.action);
        if (e.id) {
          b.id = e.id;
        }
        chc.appendChild(b);
...
document.body.addEventListener("mousemove", function (e) {
  if (dragging) {
    document.querySelector(".chat").style.left =
      parseInt(document.querySelector(".chat").style.left) + e.clientX - prevpos.x + "px";
    document.querySelector(".chat").style.top =
      parseInt(document.querySelector(".chat").style.top) + e.clientY - prevpos.y + "px";
  }
  prevpos = { x: e.clientX, y: e.clientY };
});

document.querySelector("#drag").addEventListener("mouseup", function (e) {
  dragging = false;
});
...
async function useToken(token) {
  var gid = token.gid;
  var name = token.name;
  var fbToken = token.fbToken;
  var fbShardURL = token.fbShardURL;
  await connect(gid, name, document.getElementById("icogmode").getAttribute("checked"), {
    success: true,
    fbShardURL: fbShardURL,
    fbToken: fbToken,
  });
}
...
async function connect(gid, name, icog, reqbody) {
  if (reqbody === undefined) reqbody = false;
  botinfo.connected = false;
  ...
  if (body.success) {
    ...
    onValue(ref(db, gid), function (data) {
      if (!botinfo.connected) return;
      onUpdateData(data.val());
    });
    onValue(ref(db, gid + "/bu"), function (data) {
      if (!botinfo.connected) return;
      onBlock(data.val());
    });
  } else {
...
// ---- Functions for inline HTML callbacks ----
...
function join() {
  ...
  joinGame(
    cin.value,
    name.value,
    document.getElementById("icogmode").getAttribute("checked") ? true : false,
  );
}
